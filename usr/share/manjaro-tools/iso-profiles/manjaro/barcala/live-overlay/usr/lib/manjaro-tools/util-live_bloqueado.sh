...
# Mod. por Enrique Cabo.
# - Lanzar mntsqfs.sh
# - No permitir montar discos internos rw (gab. PCs)
...
configure_user(){
	...
	if [[ -n ${password} ]];then
		...
		echo "useradd -M -G ${addgroups} -p $(gen_pw) -s ${login_shell} ${username}"
	else
		useradd -M -G ${addgroups} -s ${login_shell} ${username}
		echo "useradd -M -G ${addgroups} -s ${login_shell} ${username}"
	fi

       /usr/bin/sh /usr/lib/manjaro-tools/mntsqfs.sh
}
