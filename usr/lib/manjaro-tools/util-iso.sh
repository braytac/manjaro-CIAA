...
make_image_desktop() {
    if [[ ! -e ${work_dir}/build.${FUNCNAME} ]]; then
        ...
        \cp -f "${path}/desktopfs-pkgs.txt" ${iso_dir}/$(gen_iso_fn)-pkgs.txt
...
make_image_live() {
    ...
    chown -R 1000:1000 "${path}/home/manjaro"
    chmod -R 777 "${path}/home/manjaro/Escritorio"
    chmod -R 777 "${path}/home/manjaro/Descargas"
...
make_image_mhwd() {
    if [[ ! -e ${work_dir}/build.${FUNCNAME} ]]; then
        ...
        \cp -f ${DATADIR}/pacman-mhwd.conf ${path}/opt/mhwd
        make_repo "${path}"
...
make_image_boot() {
    if [[ ! -e ${work_dir}/build.${FUNCNAME} ]]; then
        ...
        \cp -f ${work_dir}/rootfs/boot/vmlinuz* ${boot}/vmlinuz-${target_arch}
        ...
        \cp -f ${path}/boot/initramfs.img ${boot}/initramfs-${target_arch}.img
...
configure_grub(){
    local default_args="misobasedir=${iso_name} misolabel=${iso_label}" \
        boot_args=('quiet' 'systemd.show_status=1' 'noibrs' 'noibpb' 'nopti' 'nospectre_v2' 'nospectre_v1' 'l1tf=off' 'nospec_store_bypass_disable' 'no_stf_barrier' 'mds=off' 'mitigations=off' ${apparmor_boot_args})
